// ==UserScript==
// @name         三国杀全能助手 (动态白名单+自动购买)
// @namespace    http://tampermonkey.net/
// @version      1.0.0
// @description  [1-6]购买 [R]刷新 [F]锁定 [Shift+1]遣散手牌最右 [Shift+2]使用锦囊 [Alt+9]遣散战斗区最右 [Alt+0]上阵最右 [Space]跳过 [Tab]三连开关 [Shift+R]强制刷新 [E]一键清理非目标武将 | 新增: 🛒 自动购买助手 | 新增: ⚔️ 流派一键白名单 | 修复: 一键遣散现在会自动保留同名将的所有品质（如金卡）
// @author       Assistant & AI
...
(function() {
    ...
    function deployRightmostCard() {
        const mgr = getManager();
        if (!mgr || mgr.phase !== 6 || !mgr.CanOperate) return false;
        const hand = mgr.HandChess;
        if (!hand || hand.length === 0) { showToast("手牌为空"); return false; }
        const last = hand[hand.length - 1];
        if (!last || !last.goodsID) return false;
        ...
        if (typeof mgr.ReqChessLineUp !== "function") return false;
        let pollCount = 0;
        const pollInterval = setInterval(() => {
            ...
            if (currentHand) {
                for (let i = 0; i < currentHand.length; i++) {
                    if (currentHand[i] && currentHand[i].goodsID === goodsID) { found = true; break; }
                }
            }
    ...
    function useRightmostSpell() {
        const mgr = getManager();
        if (!mgr || mgr.phase !== 6) return false;
        const hand = mgr.HandChess;
        ...
        function tryNextTarget() {
            if (isCompleted || currentTargetIdx >= targetList.length) {
                if (!isCompleted) showToast("锦囊无可用目标");
                isCompleted = true;
                mgr.off('RESP_CHESS_SPELL_USE', onSpellResponse);
                return;
            }
            if (typeof mgr.ReqChessUseSpell === 'function') {
                mgr.ReqChessUseSpell(spellGoodsID, [targetList[currentTargetIdx]]);
            }
        }
        function onSpellResponse(e) {
            if (isCompleted) return;
            if (e.Protocol && e.Protocol.errCode) {
                currentTargetIdx++;
                tryNextTarget();
            } else {
                isCompleted = true;
    ...
    // ==========================================
    // 模块 3: 自动购买助手 (数据源从 CHESS_DATA 派生)
    // ==========================================
 
    const AutoBuyHelper = {
        // name -> minId 映射 (从 CHESS_DATA 构建)
        nameToMinId: {},
        ...
        state: {
            isRunning: false,
            selectedNames: new Set(), // 存储选中的武将名称
            maxRefresh: 20,
            currentRefresh: 0,
            boughtCount: 0,
            isExpanded: false
        ...
        // 从 CHESS_DATA 构建 nameToMinId 和 CHESS_LIST
        buildDataFromChessData: function() {
            const nameMap = new Map(); // name -> minId
            ...
            while (this.state.isRunning && this.state.currentRefresh < this.state.maxRefresh) {
                ...
                for (let item of goods) {
                    if (item && item.chessID) {
                        // 检查该 chessID 是否属于任一选中武将 (匹配普通版本 ID)
        ...
        createUI: function() {
            ...
            abFactionSelect.onchange = (e) => {
                ...
                if (!names) return;
                names.forEach(name => {
                    if (this.nameToMinId[name]) { // 只有存在于数据中才添加
                        this.addToSelected(name);
                    }
                });
                this.renderSelectedList();
    ...
    function onKeyDown(e) {
        ...
        if (e.code === 'KeyR' && e.shiftKey) { e.preventDefault(); refreshHandView(undefined, "manual"); refreshBattleView("manual"); showToast("强制刷新UI"); return; }
        if (e.code === 'Digit2' && e.shiftKey) { e.preventDefault(); useRightmostSpell(); return; }
        if (e.key === 'Tab') {
            e.preventDefault();
            window.blockTripleCombine = !window.blockTripleCombine;
            showToast(window.blockTripleCombine ? "三连阻止" : "三连允许");
            if (typeof AutoBuyHelper !== 'undefined') AutoBuyHelper.updateTripleStatus();
        ...
        if (e.code === 'Digit1' && e.shiftKey) { e.preventDefault(); discardRightmostHand(); return; }
        if (e.altKey && e.key === "9") { e.preventDefault(); discardRightmostBattle(); return; }
        if (e.altKey && e.key === "0") { e.preventDefault(); deployRightmostCard(); return; }
        if (e.key.toLowerCase() === 'e' || e.code === 'KeyE') { e.preventDefault(); smartClearHand(); return; }
        if (e.key >= "1" && e.key <= "6") { e.preventDefault(); buyChess(parseInt(e.key) - 1); return; }
        if (e.key.toLowerCase() === "r") { e.preventDefault(); refreshShop(); return; }
        if (e.key.toLowerCase() === "f") { e.preventDefault(); lockShop(); return; }
        if (e.key === " " || e.key === "Space" || e.code === "Space") { e.preventDefault(); skipBattle(); }
    }
    ...
    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", () => {
            ...
            initConfigPanel();
            AutoBuyHelper.init();
            setTimeout(() => { if (typeof AutoBuyHelper !== 'undefined') AutoBuyHelper.updateTripleStatus(); }, 200);
        });
    } else {
        document.addEventListener("keydown", onKeyDown);
        setTimeout(bindEvents, 1000);
    ...
    console.info("[AutoChess Enhanced] 已启动 | 数据源已统一 | 购买助手仅匹配商店普通品质");
})();