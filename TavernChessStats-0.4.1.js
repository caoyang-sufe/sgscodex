...
// @version      0.5.0
// @description  统计酒馆自走棋本局商店卡牌出现、刷新与购买虎符消耗，支持分回合、拖拽停靠右侧和结束下载。
// @author       Codex
// @match        *://*.sanguosha.com/*
...
(function () {
    ...
    function loadPanelState() {
        const fallback = { left: Math.max(12, window.innerWidth - 340), top: Math.max(12, window.innerHeight - 470), width: 320, height: 420, docked: false, collapsed: false };
        try {
    ...
    function dockToSidebar(panel) {
        ...
        panel.style.boxShadow = '-4px 0 40px rgba(0,0,0,0.6), 0 0 0 1px rgba(242,222,156,0.12)';
        document.body.style.paddingRight = panelWidth + 'px';
        if (dockHint) dockHint.classList.remove('show');
        savePanelState(panel);
        window.dispatchEvent(new Event('resize'));
    }

    function undockFromSidebar(panel) {
        if (!isDocked) return;
        isDocked = false;
        ...
        panel.style.boxShadow = '0 6px 22px rgba(0,0,0,.35), 0 0 0 1px rgba(242,222,156,0.15)';
        savePanelState(panel);
        window.dispatchEvent(new Event('resize'));
    }
    ...
    function createDockHint() {
        if (document.getElementById('tavern-dock-hint')) return;
        const hint = document.createElement('div');
        hint.id = 'tavern-dock-hint';
        hint.style.cssText = [
            'position:fixed',
            'right:0',
            'top:0',
            ...
            'pointer-events:none',
            'z-index:99998',
            'background:linear-gradient(90deg, transparent 0%, rgba(55,40,32,0.6) 60%, rgba(55,40,32,0.8) 100%)',
            'opacity:0',
            'transition:opacity 0.3s'
        ].join(';');
        hint.innerHTML = '<span style="position:absolute;right:10px;top:50%;transform:translateY(-50%);writing-mode:vertical-rl;font-size:14px;color:rgba(242,222,156,0.6);letter-spacing:2px;font-weight:300;">拖到此处停靠</span>';
    ...
    function createPanel() {
        ...
        isDocked = state.docked || false;
        isCollapsed = state.collapsed || false;
        panelWidth = state.width || 320;
        ...
        panel.style.cssText = [
            ...
            'box-shadow:0 6px 22px rgba(0,0,0,.35), 0 0 0 1px rgba(242,222,156,0.15)',
        ...
        if (isDocked) {
            ...
            panel.style.right = state.left ? 'auto' : '20px';
        ...
        resizeHandle.id = 'tavern-resize-handle';
        ...
        resizeHandle.innerHTML = '<style>#tavern-resize-handle::after{content:"";position:absolute;right:2px;top:50%;transform:translateY(-50%);width:2px;height:40px;background:rgba(242,222,156,0.15);border-radius:2px;transition:background 0.2s;}#tavern-resize-handle:hover::after{background:rgba(242,222,156,0.35);}.docked #tavern-resize-handle{right:auto;left:-8px;border-radius:4px 0 0 4px;}.docked #tavern-resize-handle::after{right:auto;left:3px;}</style>';

        // 标题栏
        const header = document.createElement('div');
        header.id = 'tavern-chess-stats-header';
        ...
        title.textContent = isMinimized ? '📊 统计' : '自走棋统计';
        title.style.fontSize = '14px';

        const controls = document.createElement('div');
        controls.style.cssText = 'display:flex;gap:4px;';

        const btnMinimize = createButton('最小化', function() {
            const content = document.getElementById('tavern-chess-stats-content');
            isMinimized = !isMinimized;
            if (isMinimized) {
                content.style.display = 'none';
                title.textContent = '📊 统计';
                panel.style.minWidth = 'auto';
                panel.style.minHeight = 'auto';
                this.textContent = '恢复';
            } else {
                content.style.display = 'block';
                title.textContent = '自走棋统计';
        ...
        const btnExport = createButton('导出', function() { exportStats(false); });

        const btnCollapse = createButton(isCollapsed ? '展开' : '收起', function() {
            const content = document.getElementById('tavern-chess-stats-content');
        ...
        content.id = 'tavern-chess-stats-content';
        ...
        status.id = 'tavern-chess-stats-status';
        status.style.cssText = 'color:#a5b1c2;margin-bottom:6px;font-size:12px;';
        status.textContent = '初始化中...';

        const body = document.createElement('div');
        body.id = 'tavern-chess-stats-body';
        ...
        setupDockDetection(panel, header);

        // 保存引用
        window.__tavernPanel = panel;
        window.__tavernHeader = header;
    ...
    function createButton(text, onClick) {
        ...
        btn.addEventListener('mouseenter', function() { this.style.background = '#6a7a8a'; });
        btn.addEventListener('mouseleave', function() { this.style.background = '#596275'; });
    ...
    function makePanelDraggable(panel, header) {
        ...
        document.addEventListener('mousemove', function(event) {
            ...
            if (dockHint) {
                dockHint.classList.toggle('show', rightOverflow > DOCK_THRESHOLD);
            }
        });
        ...
        document.addEventListener('mouseup', function() {
            ...
            const rect = panel.getBoundingClientRect();
            const rightOverflow = rect.right - window.innerWidth;
            if (rightOverflow > DOCK_THRESHOLD) {
                dockToSidebar(panel);
            } else if (dockHint) {
    ...
    function setupResizeHandle(panel) {
        const handle = document.getElementById('tavern-resize-handle');
        if (!handle) return;
        ...
        function onResizeMove(event) {
            ...
            if (rect.right > window.innerWidth - 10) {
                panel.style.left = (window.innerWidth - newWidth - 10) + 'px';
            }
        }
    ...
    // ============ 停靠检测（鼠标移出右侧） ============
    function setupDockDetection(panel, header) {
        ...
        panel.addEventListener('dblclick', function(e) {
            if (e.target.closest('button') || e.target.closest('#tavern-resize-handle')) return;
            if (isDocked) {
                undockFromSidebar(panel);
            }
        });
    ...
    waitAndAttach();
    console.log(LOG_PREFIX, '脚本 v0.5.0 已启动 (支持侧边栏停靠)');
})();