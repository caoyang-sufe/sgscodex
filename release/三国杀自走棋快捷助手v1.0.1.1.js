...
// @version      1.0.1.1
// @description  [1-6]购买  [R]刷新  [F]锁定  [Shift+1]遣散手牌中最右侧卡牌  [Shift+2]使用最右侧锦囊(自动尝试商店->上阵)  [Shift+3]自动随征(最右侧随征卡→位置0)  [Alt+9]遣散上阵区域最右侧卡牌  [Alt+0]上阵手牌中最右侧卡牌 [Space]跳过战斗 [Tab]禁用/启用三连控制 [Shift+R]强制刷新UI | 2x速度（测试不生效，本质UI动画滞后的同步策略） | 事件+轮询刷新
// @author       鲁班大王
...
(function() {
    ...
    console.info("[AutoChess] v1.0.2 已启动 | 1-6购买  Shift+1遣散手牌最右  Shift+2使用最右侧锦囊(自动尝试商店->上阵)  [Shift+3]自动随征(最右侧随征卡→位置0)  Alt+9遣散战斗区最右  Alt+0上阵最右  R刷新  F锁定  空格跳过  Tab切换三连状态  Shift+R强制刷新UI | 2x速度 | 事件+轮询刷新");
