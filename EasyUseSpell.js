...
function useSpellWithTarget(index, targetType, targetValue) {
    ...
    if (index < 0 || index >= hand.length) {
        console.log('❌ 索引超出范围，手牌数量:', hand.length);
        return false;
    }
    ...
    switch(targetType) {
        case 'enemy':
            ...
            if (targetValue !== undefined && targetValue < enemyChess.length) {
                ...
                console.log('🎯 目标: 敌方棋子 #' + targetValue, targets);
            } else {
                console.log('❌ 敌方棋子索引无效');
                return false;
            }
            break;
        ...
        case 'self':
            ...
            if (targetValue !== undefined && targetValue < selfChess.length) {
                ...
                targets = [target.goodsID || target.GoodsID || target.UniqueId || 0];
                console.log('🎯 目标: 己方棋子 #' + targetValue, targets);
            } else {
                console.log('❌ 己方棋子索引无效');
                return false;
            }
            break;
        ...
        case 'all_enemy':
            ...
            console.log('🎯 目标: 所有敌方 (' + targets.length + '个)', targets);
            break;
        ...
        case 'all_self':
            ...
            console.log('🎯 目标: 所有己方 (' + targets.length + '个)', targets);
            break;
        ...
        case 'specific':
            ...
            if (targetValue) {
                targets = [targetValue];
                console.log('🎯 目标: 指定ID', targets);
            } else {
                console.log('❌ 需要提供目标ID');
                return false;
            }
            break;
        ...
        case 'position':
            ...
            if (posTarget.length > 0) {
                ...
                console.log('🎯 目标: 位置 ' + targetValue, targets);
            } else {
                console.log('❌ 位置 ' + targetValue + ' 没有棋子');
                return false;
            }
            break;
        ...
        case 'shop':
            ...
            if (targetValue !== undefined && targetValue < shopGoods.length) {
                ...
                console.log('🎯 目标: 商店卡牌 #' + targetValue, targets);
            } else {
                console.log('❌ 商店卡牌索引无效');
                return false;
            }
            break;

        default:
            console.log('❌ 未知目标类型:', targetType);
            console.log('  支持的类型: enemy, self, all_enemy, all_self, specific, position, shop');
            return false;
    }

    if (targets.length === 0) {
        console.log('❌ 没有找到有效目标');
        return false;
    }

    // 使用锦囊
    console.log('▶️ 使用锦囊 goodsID:', goodsID, 'spellID:', spellID, 'targets:', targets);
    ...
    if (typeof m.ReqChessUseSpell === 'function') {
        ...
        console.log('❌ ReqChessUseSpell 方法不存在');
        return false;
    }
}
...
function listTargets() {
    ...
    selfChess.forEach(function(c, i) {
        ...
        console.log('  ' + i + '.', name, 'goodsID:', c.goodsID || c.GoodsID || 0, 'pos:', c.pos || c.Pos || 0);
    });
    ...
    shopGoods.forEach(function(c, i) {
        if (c) {
            console.log('  ' + i + '.', 'goodsID:', c.goodsID || 0, 'chessID:', c.chessID || 0);
        }
    });

    console.log('💡 使用: useSpellWithTarget(手牌索引, 目标类型, 目标值)');
    console.log('  示例: useSpellWithTarget(0, "enemy", 0)  // 对敌方第1个棋子使用');
    console.log('  示例: useSpellWithTarget(1, "all_enemy")  // 对所有敌方棋子使用');
    console.log('  示例: useSpellWithTarget(2, "self", 0)    // 对己方第1个棋子使用');
    console.log('  示例: useSpellWithTarget(3, "position", 2) // 对位置2的棋子使用');
}
...
console.log('===== 需要目标的锦囊使用命令已加载 =====');
console.log('📌 命令:');
console.log('  listTargets()                          - 查看所有可选目标');
console.log('  useSpellWithTarget(手牌索引, 类型, 值) - 使用锦囊');
console.log('');
console.log('📖 目标类型:');
console.log('  "enemy", 值: 敌方棋子索引');
console.log('  "self", 值: 己方棋子索引');
console.log('  "all_enemy" (无需值)');
console.log('  "all_self" (无需值)');
console.log('  "specific", 值: goodsID');
console.log('  "position", 值: 棋盘位置(0-6)');
console.log('  "shop", 值: 商店卡牌索引');
console.log('');
console.log('💡 示例:');
console.log('  useSpellWithTarget(0, "enemy", 0)      // 对敌方第1个棋子使用');
console.log('  useSpellWithTarget(1, "all_enemy")     // 对所有敌方棋子使用');